...
export default function Stepper({
  ...
  return (
    <div className="outer-container" {...rest}>
      <div className={`step-circle-container ${stepCircleContainerClassName}`} style={{ border: '1px solid var(--color-hairline, #222)' }}>
...
function StepIndicator({ step, currentStep, onClickStep, disableStepIndicators }: any) {
  ...
  return (
    <motion.div onClick={handleClick} className="step-indicator" style={disableStepIndicators ? { pointerEvents: 'none', opacity: 0.5 } : {}} animate={status} initial={false}>
      <motion.div
        variants={{
          inactive: { scale: 1, backgroundColor: 'var(--surface-strong, #222)', color: 'var(--text-muted, #a3a3a3)' },
          active: { scale: 1, backgroundColor: 'var(--ink, #5227FF)', color: 'var(--canvas, #5227FF)' },
          complete: { scale: 1, backgroundColor: 'var(--ink, #5227FF)', color: 'var(--text-muted, #3b82f6)' }
        }}
...
function StepConnector({ isComplete }: any) {
  const lineVariants = {
    ...
    complete: { width: '100%', backgroundColor: 'var(--ink, #5227FF)' }
  };
